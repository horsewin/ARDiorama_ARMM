...
void GLWindow2::on_mouse_move(GLWindow& win, CVD::ImageRef where, int state)
{
  ...
  // Added by Umakatsu
  this->mvMousePosition[0] = where.x;
  this->mvMousePosition[1] = where.y;
  // PTAMM
...
void GLWindow2::on_mouse_up(GLWindow& win, CVD::ImageRef where, int state, int button)
{
  ...
  this->mvMousePosition[1] = where.y;
  int remove = button;
  remove &= ~BUTTON_WHEEL_UP;
  remove &= ~BUTTON_WHEEL_DOWN;
  this->miMouseState    &= ~remove;
}
...
Vector<2> GLWindow2::GetMousePosition(void){
  return mvMousePosition;
}

int GLWindow2::GetMouseState(void){
  int state = miMouseState;
  miMouseState &= ~BUTTON_WHEEL_UP;
  miMouseState &= ~BUTTON_WHEEL_DOWN;
  return state;
}
