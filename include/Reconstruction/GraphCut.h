...
private:
  ...
  /* Member variable */
	...
	VideoData * vd;
	/* 尤度を求めるための各種パラメータ */
	boost::array < TooN::Vector < D > , CLASSNUMBER> mu_fore;
	boost::array < TooN::Vector < D > , CLASSNUMBER> mu_back;
	boost::array< TooN::Matrix < D > , CLASSNUMBER > covariance_inv_fore;
	boost::array< TooN::Matrix < D > , CLASSNUMBER > covariance_inv_back;
	TooN::Vector < CLASSNUMBER >	det_covariance_fore;
	TooN::Vector < CLASSNUMBER >	det_covariance_back;
	TooN::Vector < CLASSNUMBER > constpart_fore; //同時確率密度関数の定数部
	TooN::Vector < CLASSNUMBER > constpart_back; //同時確率密度関数の定数部
