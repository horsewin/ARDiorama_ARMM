...
namespace PTAMM{
	...
	void MarchingCubes::save(const char * filename){
		...
		REP(m,texSize){
			...
			IplImage * dst_image = cvCreateImage(cvSize(WIDTH, HEIGHT), IPL_DEPTH_8U, 3);
	...
	//			}
				...
				//writing image data to IplImage variable
				REP(y, dst_image->height){
					REP(x, dst_image->width){
						dst_image->imageData[ ( y * dst_image->widthStep) + (x * 3) + 2] = buf[ (y * WIDTH + x)*3 + 0];
						dst_image->imageData[ ( y * dst_image->widthStep) + (x * 3) + 1] = buf[ (y * WIDTH + x)*3 + 1];
						dst_image->imageData[ ( y * dst_image->widthStep) + (x * 3) + 0] = buf[ (y * WIDTH + x)*3 + 2];
					}
				}
			...
			for (i = mTriangles[tnum].begin(); i != mTriangles[tnum].end(); i++) {
				...
				if( mode != 1) glBegin(GL_TRIANGLES);
					for (int j = 0; j < 3; j++) {
						...
						if( tnum < texSize){
							if(mode == 2){
								glTexCoord2d( tex[ 0 ]/WIDTH, tex[ 1 ]/HEIGHT );
							}
						}
						//テクスチャが見つからなかった場合は背景領域の座標値を割り当てる
						else{
